...
1. **[Major, IV-001]** Extend `tests/run-drift-tests.sh` to byte/structure-protect the four unprotected duplicated blocks: the command-file discovery stub (Block 1), concurrent-writes rule (Block 18), timestamp template (Block 4), test-count template (Block 5). This closes the same silent-divergence gap the task targeted.
